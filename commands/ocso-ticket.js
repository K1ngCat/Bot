...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('ocso-ticket')
        .setDescription('Issue an OCSO ticket to a user.')
    ...
    async execute(interaction) {
        ...
        try {
            if (!member.roles.cache.has(TICKET_ROLE_1)) await member.roles.add(TICKET_ROLE_1);
                ...
                try { await user.send('⚠️ You have received your third ticket role.'); } catch {}
            }
        ...
        const embed = new EmbedBuilder()
            .setTitle('🚔 Outagamie County Sheriff`s Office - Traffic Ticket')
            .setColor(0xBA8B11)
            .setDescription(
                `**Ticket #:** ${ticketNumber}\n` +
                `**Offender:** <@${user.id}>\n` +
            ...
            .setFooter({ text: 'Greenville RP | OCSO Division' });

        await interaction.reply({ content: `<@${interaction.user.id}>`, embeds: [embed] });
        try { await user.send({ content: 'You have been issued a OCSO ticket:', embeds: [embed] }); } catch {}
    }
};
